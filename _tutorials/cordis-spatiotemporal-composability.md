...
What it does establish: the paradigm exists, is implemented, and is used in a real, if single, TypeScript ecosystem — Koishi/Cordis. [DeepSeek Harness is a separate downstream application of the framework](/tutorials/deepseek-harness-and-cordis-why-everything-is-a-plugin/). Its composition model can also be contrasted with [Pi Agent's deliberately minimal extension model](/tutorials/deepseek-harness-vs-pi-agent/). The paper's evidence is observational: it demonstrates that the paradigm can be built and adopted, and it walks through revertible effects, reactive coeffects, and the Fiber lifecycle as a coherent design.
